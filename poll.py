...
class Poller(Thread):
    ...
    def _poll_sys_res(self):
        ...
        self.log_queue.append(llog.new_line(llog.LogType.CPU_USAGE, psutil.cpu_percent()));
        self.log_queue.append(llog.new_line(llog.LogType.MEM_USAGE, psutil.virtual_memory().percent));
        disk_stats = psutil.disk_io_counters();
        self.log_queue.append(llog.new_line(llog.LogType.DISK_READ, (disk_stats.read_count, disk_stats.read_bytes / (1 << 10), disk_stats.read_time)));
        self.log_queue.append(llog.new_line(llog.LogType.DISK_WRITE, (disk_stats.write_count, disk_stats.write_bytes / (1 << 10), disk_stats.write_time)));
            ...
            if self.et_proc_poll <= 0:
                ...
                for p in setdiff:
                    try:
                        ...
                        self.log_queue.append(llog.new_line(llog.LogType.NEW_PROCESS, t));
                    except:
...
class Sniffer:
    ...
    def __init__(self):
        ...
        """
        self.log_queue = [];
        self.sniffer = AsyncSniffer(prn=self._append_packet, filter='tcp and ip', store=None);
        self.sniffer.start();
    ...
    def _append_packet(self, x):
        ...
        if x.haslayer(HTTPRequest) or x[TCP].dport == 443:
            self.log_queue.append(llog.new_line(llog.LogType.NET_CAPTURE, (x[IP].src, x[IP].dst)));
