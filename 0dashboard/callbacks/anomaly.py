...
from pages.utils import create_param_table, create_metric_table, create_empty_figure
...
@callback(
    Output("anomaly-select-device", "options"),
    Output("anomaly-select-device", "value"),
    Input("anomaly-refresh-devices", 'n_clicks')
)
def refresh_devices(n_clicks):
    ...
    if prop_id == 'anomaly-refresh-devices' and n_clicks>0:
        for dev in r.keys():
...
@callback(
    ...
    Output("anomaly-select-file", "value"),
    Input("anomaly-select-device", "value"),
)
def load_data(dev):
    name = None
    options = []
...
def select_features(n_clicks, dev, train_file, test_file, label_name):
    ...
    if prop_id == "anomaly-select-features-parent":
        ...
        df = AnomalyModel().load_data(dev, train_file, nrows=2)
...
def select_label(n_clicks, dev, train_file, test_file, features):
    options = []
    ctx = dash.callback_context
    prop_id = ctx.triggered_id
    ...
    try:
        if ctx.triggered:
            ...
            if prop_id == "anomaly-train-btn" and train_clicks > 0:
                ...
                df = AnomalyModel().load_data(device, train_filename)
                if file_mode == "single":
                    ...
                    train_df = df
                    test_df = AnomalyModel().load_data(device, test_filename)
                ...
                if file_mode == "single":
                    ...
                    assert test_filename, "The test file is empty!"
                    df = AnomalyModel().load_data(device, test_filename)
